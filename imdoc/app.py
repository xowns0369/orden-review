from flask import Flask, render_template, request, abort, send_from_directory, url_for, session # 1. session 추가
import json, os
...
# 2. 세션 암호화 키 (이게 있어야 브라우저별로 '이름표'를 붙일 수 있습니다)
app.secret_key = 'imdoctor_secret_key_2026' 
...
# --- 3. 방문자 카운트 로직 (결정한 방식 반영) ---
@app.before_request
def count_visit():
    # 오직 메인 페이지('/')에 접속했을 때만 작동!
    # 그리고 이번 브라우저 세션에서 '방문 도장'이 없을 때만!
    if request.path == "/" and 'visited' not in session:
        try:
            data = load_visits()
            data["count"] = data.get("count", 0) + 1
            save_visits(data)
            
            # 세션에 도장 찍기 (브라우저를 완전히 닫기 전까지 유지됨)
...
#검단구 산부인과
    {"name": "우리들산부인과의원","category":"산부인과","gu":"검단구", "region": "왕길동", "address": "인천 서구 완정로 187 마전메디칼센타", "keywords": ["산부인과전문의"], "rating": 0,"website": "","commercial_level": 1, "specialty": [] },
    {"name": "강남산부인과의원","category":"산부인과","gu":"검단구", "region": "왕길동", "address": "인천 서구 검단로 474 뷰비스타운 4층", "keywords": ["산부인과전문의"], "rating": 0,"website": "","commercial_level":1 , "specialty": [] },
    {"name": "한사랑산부인과의원","category":"산부인과","gu":"검단구", "region": "당하동", "address": "인천 서구 원당대로 660 영프라자 6층, 7층", "keywords": ["산부인과전문의"], "rating": 0,"website": "","commercial_level":2, "specialty": [] },
    {"name": "검단서울여성의원","category":"산부인과","gu":"검단구", "region": "당하동", "address": "인천 서구 원당대로 664 5층", "keywords": ["산부인과전문의" ], "rating": 0,"website": "","commercial_level": 2, "specialty": [] },
    {"name": "뉴서울여성의원","category":"산부인과","gu":"검단구", "region": "당하동", "address": "인천 서구 청마로19번길 20 3층", "keywords": ["서울대출신 원장","산부인과전문의"], "rating": 0,"website": "https://nsh.kr/doctor","commercial_level":2, "specialty": [] },
    {"name": "검단성모산부인과의원","category":"산부인과","gu":"검단구", "region": "원당동", "address": "인천 서구 이음대로 378 1동 408, 409호", "keywords": ["산부인과전문의","대학병원출신","여성의료진"], "rating": 0,"website": "https://gdsungmo.com/","commercial_level":2, "specialty": [] },
    {"name": "애플산부인과의원 검단점","category":"산부인과","gu":"검단구", "region": "원당동", "address": "인천 서구 이음대로 388 1동 608~611호 [ABM타워]", "keywords": ["산부인과전문의","여성의료진"], "rating": 0,"website": "https://applewoman.co.kr/gd/main/main.php","commercial_level":2, "specialty": [] },
    {"name": "검단아라산부인과의원","category":"산부인과","gu":"검단구", "region": "원당동", "address": "인천 서구 이음대로 392 메트로시티 5층", "keywords": ["산부인과전문의","대학병원출신","여성의료진"], "rating": 0,"website": "https://www.araclinics.com/","commercial_level":2, "specialty": [] },
    {"name": "아라서울여성의원","category":"산부인과","gu":"검단구", "region": "원당동", "address": "인천 서구 원당대로 1045 4층, (원당동,금호헤리티지7)", "keywords": ["서울대출신 원장","산부인과전문의"], "rating": 0,"website": "http://seoulwomenclinic.co.kr/","commercial_level":1, "specialty": [] },
    {"name": "샤인산부인과의원","category":"산부인과","gu":"검단구", "region": "불로동", "address": "인천 서구 고산후로 222 3층 301-304호", "keywords": ["산부인과전문의","여성의료진"], "rating": 0,"website": "","commercial_level":2, "specialty": [] },
    # 서구 산부인과
    {"name": "금산부인과","category":"산부인과","gu":"서구", "region": "검암동", "address": "인천 서구 승학로 528", "keywords": ["산부인과전문의"], "rating": 0,"website": "","commercial_level":1, "specialty": [] },
    {"name": "이해림산부인과의원","category":"산부인과","gu":"서구", "region": "연희동", "address": "인천 서구 서곶로 355-1 4층", "keywords": ["산부인과전문의","여성의료진"], "rating": 0,"website": "","commercial_level":1, "specialty": [] },
    {"name": "김승연산부인과의원","category":"산부인과","gu":"서구", "region": "연희동", "address": "인천 서구 서곶로 355", "keywords": ["산부인과전문의"], "rating": 0,"website": "","commercial_level":1, "specialty": [] },
    {"name": "닥터스의원","category":"산부인과","gu":"서구", "region": "청라동", "address": "인천 서구 크리스탈로102번길 8-12 수린빌딩 5층~7층", "keywords": ["산부인과전문의","내과전문의","서울대출신 원장"], "rating": 0,"website": "http://dts.kr/","commercial_level":1, "specialty": [] },
    {"name": "청라여성병원","category":"산부인과","gu":"서구", "region": "청라동", "address": "인천 서구 중봉대로 602", "keywords": ["산부인과전문의","대학병원 출신","여성의료진","소아과"], "rating": 0,"website": "http://cnhospital.co.kr/","commercial_level":2, "specialty": [] },
    {"name": "나라여성의원","category":"산부인과","gu":"서구", "region": "가정동", "address": "인천 서구 가정로 451 벨라미센텀시티 935~941호", "keywords": ["산부인과전문의","인하대출신 원장","대학병원출신","여성의료진"], "rating": 0,"website": "http://nara-obgy.com/","commercial_level":2, "specialty": [] },
    {"name": "엄마와아이들소아청소년과산부인과의원","category":"산부인과","gu":"서구", "region": "가정동", "address": "인천 서구 가정로 388 KEB하나은행2,3층", "keywords": ["산부인과전문의","소아과전문의","여성의료진"], "rating": 0,"website": "","commercial_level":1 , "specialty": [] },
    {"name": "마리아산부인과","category":"산부인과","gu":"서구", "region": "가정동", "address": "인천 서구 가정로 358 마리아산부인과", "keywords": ["산부인과전문의","여성의료진","대학병원출신","경북대,부산대 출신 원장"], "rating": 0,"website": "https://www.incheonmary.com/main/main.php","commercial_level":1, "specialty": [] },
    {"name": "그린산부인과의원","category":"산부인과","gu":"서구", "region": "가좌동", "address": "인천 서구 건지로 371", "keywords": ["산부인과전문의","소아과"], "rating": 0,"website": "http://www.green032.com/","commercial_level":2, "specialty": [] }
...
PRECISION_COMMENTS = {
    "10~20대": {
        ...
        "팔꿈치": {
            "사무직": "팔꿈치는 손목과 어깨를 잇는 지렛대 역할을 하며, 미세한 반복 동작만으로도 주변 힘줄에 과부하가 걸리기 쉬운 부위입니다. 마우스를 잡을 때 팔꿈치가 공중에 떠 있거나 책상 모서리에 눌리는 자세가 지속되면 '테니스 엘보'와 유사한 통증이 발생할 수 있습니다. 팔꿈치를 책상 위에 편안하게 지지하고, 손목뿐만 아니라 팔 전체를 사용하여 마우스를 움직이는 습관을 들이세요.",
        ...
        "등/날개뼈": {
            "사무직": "등과 날개뼈 주변 근육은 흉추를 지지하고 올바른 상체 자세를 유지하는 핵심적인 역할을 수행합니다. 모니터에 집중하며 어깨를 앞으로 말면 날개뼈 사이 근육이 과하게 늘어나 피로해지고, 이는 등 전체의 뻐근함으로 이어집니다. 의식적으로 가슴을 펴 날개뼈를 등 가운데로 모아주는 동작을 반복하고, 등받이에 날개뼈 아래쪽까지 밀착하여 앉는 습관을 들여야 합니다.",
    ...
    "30~40대": { 
        ...
        "허리": {
            "사무직": "앉아 있을 때 허리가 받는 압력은 서 있을 때의 2배, 구부정할 땐 3배까지 치솟습니다. 30~40대의 고질병인 허리 디스크는 대부분 의자 끝에 걸터앉는 습관에서 시작됩니다. 엉덩이를 의자 깊숙이 밀착시키고, 무너진 허리 곡선을 살려주는 기능성 쿠션을 활용하세요. 1시간마다 반드시 일어나 1~2분간 제자리 걷기를 하여 척추 마디의 압력을 분산해야 합니다.",
    ...
    "50~60대": {
        ...
        "현장직": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 무거운 장비를 들기 전 반드시 가슴을 펴서 전체 정렬을 맞추는 것이 우선이며, 고개를 위로 과하게 젖히는 작업은 피하여 경추 신경의 통로가 좁아지지 않도록 주의해야 합니다.",
        "운동선수": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 운동 전에는 흉추(등)의 가동성을 충분히 확보하여 전체적인 척추 정렬을 바로잡는 예열 과정에 집중하세요. 기초 정렬이 바로 서야 목 디스크가 보상 작용으로 눌리는 부상을 막을 수 있습니다.",
        ...
        "주부/은퇴": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 가사 노동 시 턱을 당기고 가슴을 펴서 등과 목이 조화롭게 이어지는 정렬을 만드는 것이 무엇보다 중요합니다. 무거운 물건을 들 때도 억지로 허리를 쓰기보다 전체 척추의 지지력을 활용해 디스크의 부담을 덜어주세요."
        },
       "허리": {
    ...
    "70대 이상": {
       "목": {
        "사무직": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 구부정한 등은 목을 앞으로 빠지게 해 신경 압박을 심화시키므로, 평소 등을 곧게 펴서 머리의 무게가 경추 마디마디에 골고루 분산되도록 정렬을 바로잡아야 합니다. 주변 근육을 길러 이 바른 정렬을 스스로 버틸 수 있는 힘을 만들고, 고개를 갑자기 돌리거나 젖히는 동작 대신 몸 전체를 함께 움직여 관절을 보호하세요.",
        "현장직": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 구부정한 등은 목을 앞으로 빠지게 해 신경 압박을 심화시키므로, 평소 등을 곧게 펴서 머리의 무게가 경추 마디마디에 골고루 분산되도록 정렬을 바로잡아야 합니다. 주변 근육을 길러 이 바른 정렬을 스스로 버틸 수 있는 힘을 만들고, 고개를 갑자기 돌리거나 젖히는 동작 대신 몸 전체를 함께 움직여 관절을 보호하세요.",
        "운동선수": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 구부정한 등은 목을 앞으로 빠지게 해 신경 압박을 심화시키므로, 평소 등을 곧게 펴서 머리의 무게가 경추 마디마디에 골고루 분산되도록 정렬을 바로잡아야 합니다. 주변 근육을 길러 이 바른 정렬을 스스로 버틸 수 있는 힘을 만들고, 고개를 갑자기 돌리거나 젖히는 동작 대신 몸 전체를 함께 움직여 관절을 보호하세요.",
        "학생": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 구부정한 등은 목을 앞으로 빠지게 해 신경 압박을 심화시키므로, 평소 등을 곧게 펴서 머리의 무게가 경추 마디마디에 골고루 분산되도록 정렬을 바로잡아야 합니다. 주변 근육을 길러 이 바른 정렬을 스스로 버틸 수 있는 힘을 만들고, 고개를 갑자기 돌리거나 젖히는 동작 대신 몸 전체를 함께 움직여 관절을 보호하세요.",
        "주부/은퇴": "척추 전체의 정렬이 무너진 상태에서 억지로 목의 C커브만 만들려고 뒤로 꺾는 동작이 오히려 디스크 압박을 높여 신경을 건드릴 수 있습니다. 굽은 등을 먼저 펴고 턱을 가볍게 당겨 척추의 기초 정렬부터 바로잡는 것이 가장 안전한 관리법입니다. 구부정한 등은 목을 앞으로 빠지게 해 신경 압박을 심화시키므로, 평소 등을 곧게 펴서 머리의 무게가 경추 마디마디에 골고루 분산되도록 정렬을 바로잡아야 합니다. 주변 근육을 길러 이 바른 정렬을 스스로 버틸 수 있는 힘을 만들고, 고개를 갑자기 돌리거나 젖히는 동작 대신 몸 전체를 함께 움직여 관절을 보호하세요."
        },
        "허리": {
...
# 기간별 고정 팁
DURATION_TIPS = {
    "급성": " 현재는 염증기일 수 있습니다. 무리한 운동이나 스트레칭은 오히려 조직 손상을 키울 수 있으니 '냉찜질'과 '절대 안정'을 최우선으로 하세요.",
    "만성": " 주변 근육의 유착과 약화가 원인입니다. '온찜질'로 혈액순환을 돕고, 몸이 굳지 않게 통증이 없는 범위 내에서 조금씩 가동 범위를 늘려나가는 것이 중요합니다."
}
...
def result():
    # 1. 문진 데이터 수집 (복수 선택 대응)
    selected_parts = request.form.getlist("part") 
    u_age = request.form.get("age")
    u_job = request.form.get("job")
    u_dur = request.form.get("duration")
    
    # [수정] 데이터 조회용 직업 변수 생성 (기타 -> 사무직 매핑)
    u_job_query = "사무직" if u_job == "기타" else u_job
    
    user_data = {
        "age": u_age, 
        "job": u_job,  # 화면에는 유저가 선택한 원본 출력
        "duration": u_dur, 
        "parts": selected_parts,
        "part": ", ".join(selected_parts)
    }
    
    reviews = load_reviews()
    matched_hospitals = []

    # 2. 부위별 정밀 코멘트 및 병원 검색 키워드 생성
    analysis_results = [] 
    all_search_keywords = set() 

    for part in selected_parts:
        # [수정] u_job 대신 u_job_query를 사용하여 사무직 데이터 참조
        comment_text = PRECISION_COMMENTS.get(u_age, {}).get(part, {}).get(u_job_query, "")
        if not comment_text:
            comment_text = f"{u_age} {u_job_query} 가이드를 바탕으로 {part} 증상을 분석 중입니다."
        
        analysis_results.append({"part": part, "comment": comment_text})

        if part == "등/날개뼈":
            all_search_keywords.update(["등", "날개뼈"])
        elif part == "발목발가락":
            all_search_keywords.update(["발목", "발가락"])
        elif part == "손목손가락":
            all_search_keywords.update(["손목", "손가락"])
        else:
            all_search_keywords.add(part)

    # 3. 직업별/기간별 공용 팁 (조회용 직업 기준)
    job_common_tip = PRECISION_COMMENTS.get(u_job_query, "")
    duration_tip = DURATION_TIPS.get(u_dur, "")

    # 4. 병원 매칭 및 데이터 가공 (복수 키워드 대응)
    for h in hospitals:
        relevant_reviews = []
        for r in reviews:
            if r["hospital_name"] == h["name"]:
                keywords_list = r.get("keywords") or []
                if any(sk in keywords_list if sk in ["등", "목"] else any(sk in k for k in keywords_list) 
                       for sk in all_search_keywords):
        ...
        if relevant_reviews:
            ...
            display_parts = ", ".join(selected_parts)
            h_copy["match_message"] = f"{display_parts} 관련 리뷰가 {len(relevant_reviews)}개 있습니다."
    ...
    return render_template(
        ...
        comment=analysis_results[0]['comment'] if analysis_results else "",
        job_common_tip=job_common_tip,
        duration_tip=duration_tip,
        video_url=""
    )
...
def index():
    ...
    # [추가] 상단 레이어에서 선택한 키워드 파라미터
    selected_keyword = request.args.get("keyword", "")

    show_notice = bool(search_query or gu or category or region or sort_option or selected_keyword)
    reviews = load_reviews()
    ...
    for h in hospitals:
        ...
        # --- [추가/수정] 해당 병원의 리뷰 기반 키워드 통계 추출 ---
        h_reviews = [r for r in reviews if r["hospital_name"] == h["name"]]
        
        keyword_counts = {}
        for r in h_reviews:
            # 리뷰 내 keywords 리스트를 돌며 카운팅
            for k in r.get("keywords", []):
                keyword_counts[k] = keyword_counts.get(k, 0) + 1
        
        # 빈도수 기준 내림차순 정렬 후 상위 3개만 추출 (대표님 기획 반영)
        sorted_keys = sorted(keyword_counts.items(), key=lambda x: x[1], reverse=True)
        h_copy["top_keywords"] = [{"label": k, "count": v} for k, v in sorted_keys[:4]]
        # -------------------------------------------------------

        avg = calculate_average_rating(h["name"], reviews)
        h_copy["rating"] = avg if avg is not None else "평점 없음"
        h_copy["review_count"] = len(h_reviews)
        display_hospitals.append(h_copy)

    # 필터링 로직 업데이트 (키워드 필터 추가)
    filtered = [h for h in display_hospitals if 
                ...
                (not region or h["region"] == region) and
                # [추가] 선택된 키워드가 해당 병원의 TOP 3 안에 있는 경우만 노출
                (not selected_keyword or any(selected_keyword == tk['label'] for tk in h["top_keywords"]))]

    # 정렬 로직
    if sort_option == "rating":
        filtered.sort(key=lambda x: (x["rating"] == "평점 없음", -(x["rating"] if x["rating"] != "평점 없음" else 0)))
    # [추가] 키워드 선택 시 개수 많은 순으로 정렬
    elif selected_keyword:
        filtered.sort(key=lambda x: next((tk['count'] for tk in x['top_keywords'] if tk['label'] == selected_keyword), 0), reverse=True)
    else:
    ...
    return render_template(
        ...
        selected_keyword=selected_keyword, # [추가] 템플릿에 전달
